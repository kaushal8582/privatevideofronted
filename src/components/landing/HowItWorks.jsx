        ...
        <ol className="space-y-4 sm:space-y-6">
          {STEPS.map(({ num, icon: Icon, title, body }, i) => (
            <Reveal key={num} delay={i * 80}>
              <li className="relative flex gap-3 sm:gap-6 sm:pl-4">
                <div className="hidden sm:flex absolute left-0 top-6 h-4 w-4 rounded-full app-gradient-bg ring-4 ring-[var(--background)] z-10" />
                <article className="flex-1 min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-6 flex gap-3 sm:gap-4 hover:border-[var(--border-green)] transition-colors">
                  <span className="font-[family-name:var(--font-display)] text-xl sm:text-2xl font-bold text-[var(--primary)]/40 shrink-0">
                    {num}
                  </span>
