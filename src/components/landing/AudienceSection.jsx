      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
        {AUDIENCES.map(({ icon: Icon, title, body }, i) => (
          <Reveal key={title} delay={i * 60}>
            <article className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5 h-full hover:border-[var(--border-green)] transition-colors">
              <Icon className="w-6 h-6 text-[var(--primary)] mb-3 sm:mb-4" aria-hidden />
